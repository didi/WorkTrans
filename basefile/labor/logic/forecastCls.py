...
class ForecastMHandler(RequestHandler):
    @gen.coroutine
    def post(self):
    ...
    def check_param(input_json_string: str) -> Tuple[int, str]:
        ...
        # if 'nodeTimeArr' not in raw_data or  (not isinstance(raw_data['nodeTimeArr'], list)):
        #     return 10, '需要包含nodeTimeArr，并且为list类型'

        if 'taskArr' not in raw_data or  (not isinstance(raw_data['taskArr'], list)):
            return 11, '需要包含taskArr，并且为list类型'

        if len(raw_data.get('taskArr',[]))==0 :
            return 14, 'taskArr必须不为空'

        for dd  in  raw_data['taskArr']:
