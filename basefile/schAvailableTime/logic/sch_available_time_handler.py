...
@file: sch_available_time_handler.py
@time: 2020/3/20 16:11
@desc:
'''
import json
...
class SchAvailableTimeHandler(RequestHandler):

    def post(self, route, *args, **kwargs):
        infoLog.info('handler name: %s; route: %s; request body: %s' % (
            str("SchAvailableTimeHandler"), str(route), str(self.request.body.decode('utf-8')).replace('\n', '')))
        if route == "list":
    ...
    def list(self):
        ...
        return SchAvailableTimeService.list(cid, eid, page, size)
    ...
    def select(self):
        ...
        return SchAvailableTimeService.select(cid, eid_arr, day_arr)
